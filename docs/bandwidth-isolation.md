...
that overhead rather than estimating it from application payloads.
...
- `capped` adds nearby cold presence and nearest-five detailed motion.
...
- `dense` places every bot in one zone and steers rapidly while retaining the
  five-actor cap.
...
The client chooses from actors whose cold identities are already in its nearby
interest area. Existing selections receive 20% distance hysteresis so the fifth
and sixth actors do not alternate every map snapshot. A newly selected avatar
is rendered only after its first addressed detail frame arrives; all unselected
players remain visible on the minimap.
...
3. High `capped`: combine map and detail publication cadence, or reduce detail
   correction cadence only after mobile prediction-error traces stay bounded.
