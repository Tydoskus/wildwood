...
- “Boss reward audit” reports first-clear power per minute against the best regular reward cycle, then reports the permanent power available from a post-clear repeat (currently zero by contract). The explicit `boss-farm` strategy still repeats a defeated boss until the simulation window ends, with repeat kills and repeat time shown separately. Rare authored item outcomes are included in the repeat-power diagnostic so they cannot hide behind the first-clear stat ledger.
...
- Boss-rush behavior cycles the five complementary reward tracks from Lava Lake onward once its boss is reachable, repeats each cleared boss once before advancing, and repeats the final boss for the rest of the window. This represents a reward-focused route with meaningful boss loops rather than an all-damage exploit. The first clear is the progression event; repeats are a bounded-power engagement/loot choice, and the enemy table plus reward audit expose any repeat item power that changes that assumption.
