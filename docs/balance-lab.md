...
- Boss-rush behavior cycles the five complementary reward tracks from Lava Lake onward once its boss is reachable, repeats each cleared boss once before advancing, and repeats the final boss for the rest of the window. This represents a reward-focused route with meaningful boss loops rather than an all-damage exploit. The first clear is the full progression payout; repeats remain repeatable at a calibrated lower rate, and the enemy table plus reward audit expose any repeat item power that changes that assumption.
