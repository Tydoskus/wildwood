...
- Moderation review is manual, in Developer tools → Reports. `get_dev_review_queue` returns every pending `chat_message_report` and `player_report` (oldest first) and `bug_report`, then recent reviewed ones; `dev_review_report` and `dev_review_bug` record each decision (removed, dismissed, muted, banned, reopened; resolved, won't fix, duplicate) with the reviewer and a note in the private `dev_report_review` table, and a decision on one message closes every pending report about it. All of them require the database owner or the developer's signed-in game session. A decision can mail the reporter one letter (private `player_mail`, one per report, written in the decision's transaction); a player-report letter says only whether action was taken, never the penalty or the other player. The rest of the console is `get_dev_console` (muted and banned now, headline counts), `get_dev_player_card` (standing, earlier names from the log and reports, reports by and against, history, recent world/guild/DM messages), `get_moderation_log` (filtered search), `dev_warn_player` (a logged warning letter; not a chat strike, so it never triggers an automatic mute) and `dev_reset_display_name` (the moderation rename, forced). Every developer action lands in `moderation_action`. These controls do not constitute an automated review service or complete store-compliance review.
