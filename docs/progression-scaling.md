...
- **Regular enemies use one readable visual family per map.** Reuse one compact body silhouette for ordinary enemies, add a bow only for ranged variants, and increase that same silhouette for elites. Differentiate map families with authored color treatment rather than multiplying unrelated sprite sets.
