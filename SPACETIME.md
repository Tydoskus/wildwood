...
Developer Tools → Controls accepts 1–3,000 virtual players. Each bot is a real anonymous websocket client in the developer's browser: it registers the live protocol, claims a private run-scoped developer ticket through its own confirmed socket, enters the current map, installs the normal core/nearby-frame/map-frame subscriptions, simulates movement locally at 10 Hz, sends sparse state changes plus an approximately 1 Hz heartbeat, and submits a jittered save about every 2.5 seconds. Startup uses a bounded 16-client pool, waits for reducer and subscription acknowledgements, retries transient failures three times, and slows automatically as acknowledgement latency or failures rise. Bots skip the on-demand leaderboard snapshot because normal clients do not request it during join. A private per-owner counter enforces the limit without repeated full-table scans. The private `virtual_player` tag excludes bots from access history and leaderboard refreshes. Stop, socket disconnect, and maintenance all converge on the same cleanup function, which deletes every simulated profile, save, lifetime, research, static presence, motion mapping, session, and ranking row.
