...
Boss HP, accepted attack cadence/range, contributions, rewards, deaths, and respawns remain server-authoritative. A hidden global metronome derived from estimated server time controls ability order/phase; encounter seeds vary deterministic target selection and pattern geometry without shifting that rhythm. Targets use consensus-time player positions already replicated to the client. A nearby remote player's boss throw is presentation-only and shares the real attacker's absolute per-player slot. `boss_attack_frame` remains an inert compatibility table because deleting it would require a destructive publish; reducers must not insert into it and clients must not subscribe to it. This removes one event write plus map fan-out from every accepted boss attack without trusting the client for boss outcomes.
