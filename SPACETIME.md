...
Developer Tools → Controls accepts 1–3,000 virtual players. Each bot is a real anonymous websocket client in the developer's browser: it registers the live protocol, enters the current map, installs the normal core/nearby/minimap subscriptions, sends 15 Hz movement, and submits a jittered save about every 2.5 seconds. This intentionally creates real clustered-player fanout and becomes extremely expensive at high counts. A private per-owner counter enforces the limit without repeated full-table scans. The private `virtual_player` tag excludes bots from access history and leaderboard refreshes. Stop, socket disconnect, and maintenance all converge on the same cleanup function, which deletes every simulated profile, save, lifetime, research, presence, marker, session, and ranking row.
