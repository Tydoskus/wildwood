...
export function createGameUiRuntime(d: Record<string, any>) {
  ...
  const startup = createStartupController({
    accountState: () => coop?.accountState?.(), connected: () => Boolean(coop?.isConnected?.()), knownCharacter: () => coop?.knownCharacter?.() ?? "", knownCharacterGender: () => coop?.knownCharacterGender?.() ?? 0, defaultPlayerName: () => coop?.localDisplayName?.() ?? "WANDERER",
