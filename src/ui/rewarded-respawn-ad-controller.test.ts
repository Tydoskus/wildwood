...
describe("rewarded respawn countdown", () => {
  it.each(["rejected", "unavailable"])("allows retry after a %s native load and refreshes when back online", async (failure) => {
    ...
    const elements = {
      button: fakeElement(true), status: fakeElement(), bankButton: fakeElement(true),
      bankTimer: fakeElement(), prompt: fakeElement(true), confirmButton: fakeElement(),
    ...
    const controller = createRewardedRespawnAdController(elements as unknown as Parameters<typeof createRewardedRespawnAdController>[0], {
      ...
      grantBoost: activateBoost, toggleBoost: () => true, isBoostEnabled: () => false, boostRemainingMs: () => 0,
      setPromptActive: vi.fn(), setAdPlaybackActive: vi.fn(), showMessage: vi.fn(),
    });
    controller.init();
  ...
  it("banks a supporter boost on tap with no prompt and no ad", async () => {
    const runtime = Object.assign(new EventTarget(), {
      requestAnimationFrame: (callback: FrameRequestCallback) => { callback(0); return 1; },
      setTimeout: vi.fn(() => 1), clearTimeout: vi.fn(),
    });
    ...
    const controller = createRewardedRespawnAdController(elements as unknown as Parameters<typeof createRewardedRespawnAdController>[0], {
      ...
      isSupporter: () => true,
      grantBoost: () => { boosted = true; return activateBoost(); }, toggleBoost: () => true,
      isBoostEnabled: () => boosted, boostRemainingMs: () => boosted ? 30 * 60 * 1_000 : 0,
      setPromptActive: vi.fn(), setAdPlaybackActive: vi.fn(), showMessage,
    });
    controller.init();
  ...
  it("opens a confirmation prompt before starting the browser ad", () => {
    ...
    const controller = createRewardedRespawnAdController({
      button,
      status: fakeElement(),
      bankButton: fakeElement(true),
      bankTimer: fakeElement(),
      prompt,
      confirmButton,
      cancelButton,
      browserAd,
  ...
  it("shows the bank as a switch once it holds time, and spends nothing to flip it", () => {
    vi.stubGlobal("window", {
      addEventListener: vi.fn(),
      removeEventListener: vi.fn(),
      requestAnimationFrame: (callback: FrameRequestCallback) => { callback(0); return 1; },
    ...
    const button = fakeElement(false);
    const bankButton = fakeElement(true);
    const bankTimer = fakeElement();
    const showMessage = vi.fn();
    let enabled = false;
    const controller = createRewardedRespawnAdController({
      button, status: fakeElement(), bankButton, bankTimer,
      ...
      getNativeBridge: () => null,
      grantBoost: vi.fn(() => true),
      toggleBoost: () => { enabled = !enabled; return enabled; },
      isBoostEnabled: () => enabled,
      boostRemainingMs: () => 12 * 60 * 1_000,
      setPromptActive: vi.fn(),
      setAdPlaybackActive: vi.fn(),
      showMessage,
    });
    ...
    expect(bankButton.hidden).toBe(false);
    expect(bankTimer.textContent).toBe("12:00");
    expect(bankButton.getAttribute("aria-pressed")).toBe("false");

    bankButton.dispatchEvent(new Event("click"));

    expect(enabled).toBe(true);
    expect(bankButton.getAttribute("aria-pressed")).toBe("true");
    expect(bankButton.dataset.state).toBe("on");
    expect(showMessage).toHaveBeenLastCalledWith("2\u00d7 ENEMY RESPAWN ON", expect.any(String));

    bankButton.dispatchEvent(new Event("click"));

    expect(enabled).toBe(false);
    expect(bankTimer.textContent).toBe("12:00");
    expect(showMessage).toHaveBeenLastCalledWith("2\u00d7 ENEMY RESPAWN PAUSED", expect.any(String));
    controller.destroy();
  });
});
