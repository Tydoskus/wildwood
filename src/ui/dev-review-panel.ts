import { REVIEW_NOTE_MAX_LENGTH, type DevBugEntry, type DevReportEntry, type DevReviewDecision, type DevReviewQueue } from "../../shared/dev-review";
...
export type DevReviewApi = {
  ...
  reviewReport: (reportKey: string, decision: string, note: string) => Promise<ActionResult>;
  reviewBug: (id: string, decision: string, note: string) => Promise<ActionResult>;
...
export function createDevReviewPanel(containers: { reports: HTMLElement; bugs: HTMLElement }, dependencies: DevReviewPanelDependencies) {
  ...
  function decisionList(decisions: DevReviewDecision[]) {
    ...
    for (const decision of decisions) {
      ...
      item.textContent = `${decisionLabel(decision.decision)} · ${decision.reviewerName} · ${relativeTime(decision.reviewedAtMs, now())}${decision.note ? ` — ${decision.note}` : ""}`;
      list.append(item);
    }
    return list;
  }
  ...
  function reportCard(entry: DevReportEntry) {
    ...
    article.append(head(entry.status, channelLabel(entry.channel), entry.reportedAtMs), who, textBlock(entry.key, entry.text));
    ...
    const review = (decision: string, success: string) => act(entry.key, success,
      async () => api ? api.reviewReport(entry.key, decision, note(entry.key)) : { ok: false, error: "Not connected." });
    ...
    for (const choice of MUTE_CHOICES) buttons.push(action(choice.label, "plain", () => {
      void act(entry.key, `${entry.targetName} muted`, async () => {
        ...
        return muted?.ok ? api.reviewReport(entry.key, choice.decision, note(entry.key)) : muted;
      });
    }));
  ...
  function banRow(entry: DevReportEntry, api: DevReviewApi | null) {
    const row = actionRow(...BAN_CHOICES.map(choice => action(choice.label, "danger", async () => {
      ...
      void act(entry.key, `${entry.targetName} banned ${choice.words}`, async () => {
        ...
        return banned?.ok ? api.reviewReport(entry.key, "banned", note(entry.key) || choice.label) : banned;
      });
    })));
  ...
  function bugCard(entry: DevBugEntry) {
    ...
    const review = (decision: string, success: string) => act(key, success,
      async () => api ? api.reviewBug(entry.id, decision, note(key)) : { ok: false, error: "Not connected." });
  ...
  return {
    ...
    clear() {
      generation++;
      queue = null;
      notes.clear(); expanded.clear(); banOpen.clear(); busy.clear();
