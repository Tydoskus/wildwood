import { CLOUDSPIRE_MAP_ID, INFERNAL_DEPTHS_MAP_ID, MOONFEN_MAP_ID, CRYSTAL_HOLLOWS_MAP_ID, CLOCKWORK_RUINS_MAP_ID, DUSKFALL_ORCHARD_MAP_ID, NEON_BASTION_MAP_ID, SAMURAI_GARDEN_MAP_ID, WATER_REACH_MAP_ID, type MapId } from "../world";
...
export function musicSourceForMap(mapId: MapId, desertMapId: MapId, snowMapId: MapId, lavaMapId: MapId) {
  ...
  if (mapId === CLOCKWORK_RUINS_MAP_ID) return NIGHT_FOREST_MUSIC_SOURCE; else if (mapId === NEON_BASTION_MAP_ID) return NIGHT_FOREST_MUSIC_SOURCE; else if (mapId === DUSKFALL_ORCHARD_MAP_ID) return NIGHT_FOREST_MUSIC_SOURCE; else if (mapId === CRYSTAL_HOLLOWS_MAP_ID) return NIGHT_FOREST_MUSIC_SOURCE;
