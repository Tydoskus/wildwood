...
import {
  ...
  BOSS_ARMOR_REWARD_FRACTION,
  BOSS_REGEN_REWARD_FRACTION,
  DRAGON_MAX_HP,
  FROSTCLAW_MAX_HP,
  ...
  CRYSTAL_HOLLOWS_BOSS_HEALTH_CORRECTION,
...
describe("Frostclaw boss", () => {
  it("follows the shared Snowlands boss health and reward budget", () => {
    ...
    expect(FROSTCLAW_REWARD_ARMOR).toBe(FROSTCLAW_MAX_HP * BOSS_ARMOR_REWARD_FRACTION);
  });
...
describe("Magmalisk boss", () => {
  it("follows the shared Lava Lake boss health and four-stat reward budget", () => {
    ...
    expect(MAGMALISK_REWARD_ARMOR).toBe(MAGMALISK_MAX_HP * BOSS_ARMOR_REWARD_FRACTION);
    expect(MAGMALISK_REWARD_REGEN).toBe(MAGMALISK_MAX_HP * BOSS_REGEN_REWARD_FRACTION);
  });
...
describe("Gloomroot boss", () => {
  it("caps Night Forest with a four-stat Water Reach unlock reward", () => {
    ...
    expect(GLOOMROOT_REWARD_ARMOR).toBe(GLOOMROOT_MAX_HP * BOSS_ARMOR_REWARD_FRACTION);
    expect(GLOOMROOT_REWARD_REGEN).toBe(GLOOMROOT_MAX_HP * BOSS_REGEN_REWARD_FRACTION);
  });
...
describe("Tidewyrm boss", () => {
  it("caps Water Reach with the scaled Samurai Garden unlock reward", () => {
    ...
    expect(TIDEWYRM_REWARD_ARMOR).toBe(TIDEWYRM_MAX_HP * BOSS_ARMOR_REWARD_FRACTION);
    expect(TIDEWYRM_REWARD_REGEN).toBe(TIDEWYRM_MAX_HP * BOSS_REGEN_REWARD_FRACTION);
  });
...
describe("Koi Shogun boss", () => {
  it("caps Samurai Garden with a repeatable late-map reward", () => {
    ...
    expect(KOI_SHOGUN_REWARD_ARMOR).toBe(KOI_SHOGUN_MAX_HP * BOSS_ARMOR_REWARD_FRACTION);
    expect(KOI_SHOGUN_REWARD_REGEN).toBe(KOI_SHOGUN_MAX_HP * BOSS_REGEN_REWARD_FRACTION);
  });
...
describe("Tempest Kirin boss", () => {
  it("caps Cloudspire with the next repeatable late-map reward", () => {
    ...
    expect(TEMPEST_KIRIN_REWARD_ARMOR).toBe(TEMPEST_KIRIN_MAX_HP * BOSS_ARMOR_REWARD_FRACTION);
    expect(TEMPEST_KIRIN_REWARD_REGEN).toBe(TEMPEST_KIRIN_MAX_HP * BOSS_REGEN_REWARD_FRACTION);
  });
...
describe("Miremaw boss", () => {
  it("caps Moonfen with the next repeatable late-map reward", () => {
    ...
    expect(MIREMAW_REWARD_ARMOR).toBe(MIREMAW_MAX_HP * BOSS_ARMOR_REWARD_FRACTION);
    expect(MIREMAW_REWARD_REGEN).toBe(MIREMAW_MAX_HP * BOSS_REGEN_REWARD_FRACTION);
  });
...
describe("Prismshell boss", () => {
  it("continues the full progression step after Miremaw", () => {
    expect(PRISMSHELL_MAX_HP).toBe(MIREMAW_MAX_HP * BALANCE_TARGET_MAP_POWER_MULTIPLIER * 1.1 * CRYSTAL_HOLLOWS_BOSS_HEALTH_CORRECTION);
    ...
    expect(PRISMSHELL_REWARD_ARMOR).toBe(PRISMSHELL_MAX_HP * BOSS_ARMOR_REWARD_FRACTION);
    expect(PRISMSHELL_REWARD_REGEN).toBe(PRISMSHELL_MAX_HP * BOSS_REGEN_REWARD_FRACTION);
  });
