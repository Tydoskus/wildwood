...
export function createBossRenderer(options: {
  ...
  function drawBossTelegraphs() {
    ...
    if (boss.cone) {
      ...
      const waveRadius = boss.r + (BOSS_CONE_RANGE - boss.r) * clamp(1 - cone.timer / cone.duration, 0, 1);
  ...
  function drawBoss() {
    ...
    ctx.save(); ctx.textAlign = "center"; ctx.textBaseline = "middle"; ctx.font = '900 11px "Arial Rounded MT Bold", "Arial Rounded MT", Arial, sans-serif'; options.outlinedText(`${formatCompactNumber(Math.max(0, Math.ceil(boss.hp)))} / ${formatCompactNumber(Math.ceil(boss.maxHp))} HP`, x, barY + barH / 2, "#fff", 3); ctx.textBaseline = "bottom"; options.outlinedText("DRAGON", x, barY - 18, "#f5e9c4", 3); options.outlinedText("+650 DAMAGE", x, barY - 5, "#ff655a", 3); ctx.restore();
  }
