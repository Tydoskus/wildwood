...
/** Warm paper-doll preview used by the inventory loadout. */
export function createInventoryCharacterPreview(
  ...
  function draw({ visible, inventory, skinTone }: InventoryCharacterPreviewOptions) {
    ...
    const paper = ctx.createRadialGradient(width / 2, height * .42, 6, width / 2, height * .48, Math.max(width, height) * .72);
    paper.addColorStop(0, "#fffdf5");
    paper.addColorStop(1, "#eee2ca");
    ctx.fillStyle = paper;
    ctx.fillRect(0, 0, width, height);

    ctx.fillStyle = "rgba(126, 99, 59, .08)";
