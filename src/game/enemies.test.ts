...
describe("enemy sprite loading", () => {
  it("uses all original families before adding distinct new families to the remaining maps", () => {
    expect(MAP_ENEMY_FAMILIES).toEqual({
      ...
      samurai_garden: "flower-tulip", cloudspire: "wingdemon-bee", moonfen: "fungus-rock", crystal_hollows: "hornrabbit-crystal", clockwork_ruins: "raptor-mechanic", duskfall_orchard: "pumpkin-orange", neon_bastion: "neon-sentry", verdant_catacombs: "verdant-crypt",
    });
    expect(new Set(Object.values(MAP_ENEMY_FAMILIES)).size).toBe(14);
