...
describe("portal destination presentation", () => {
  it("maps Forest, Desert, Snowlands, and Lava to their destination-colored swirls", () => {
    ...
    expect(PORTAL_SWIRL_SOURCES[INFERNAL_DEPTHS_MAP_ID]).toContain("-red.png");
  });
  ...
  it("uses matching destination colors for map markers", () => {
    ...
    expect(portalDestinationColor(INFERNAL_DEPTHS_MAP_ID)).toBe("#ff3f57");
  });
});
