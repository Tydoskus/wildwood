...
(() => {
  ...
  const playerIdentityRenderer = createPlayerIdentityRenderer({
    ...
    // Clients subscribe only to their own prestige row, so a remote player's
    // level is not on the wire yet and reads as zero.
    isDeveloper: isDeveloperIdentity,
    ...
    healthBarRadius: WORLD_HEALTH_BAR_RADIUS,
  });
  const {
    publicPlayerName,
    renderDomPlayerName,
  ...
  const profileWindow = createProfileWindowController({
    ...
    renderStats: (profile, element, viewer) => renderProfileStats(profile, element, formatArmorReduction, MIN_ATTACK_INTERVAL, profile.research,
      profile.identity === coop?.localIdentity?.() ? coop?.prestige?.()?.level ?? 0 : 0,
      profile.identity === coop?.localIdentity?.() ? coop?.prestigePerks?.() : null, viewer),
