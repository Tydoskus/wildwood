...
`art-source/2D Avatar Frame/patreon_*_v2.png` (the square set). Each tier has its own artwork;
...
was: silver 12.8%, gold 10.3%, diamond 12.5%. Re-measure it if the art changes.
...
Encode once, straight to file. Running an already-encoded WebP back through
sharp re-encodes it at default quality and visibly softens the metal gradients
and the alpha edge.
